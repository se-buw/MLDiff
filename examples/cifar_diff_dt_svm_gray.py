...
from examples.cifar import load_cifar10_grayscale
...
faces = load_cifar10_grayscale(data_dir="data/cifar-10-batches-py")
X, y = faces.data, faces.target
preprocessing = PCA(n_components=123)
preprocessing.fit(X)
...
if s.check() == sat:
    ...
    for m in all_smt(s, [clDt]):
        ...
        # TODO: measure euclidean distance between face and each element of X
        distance = np.linalg.norm(faces.data - face, axis=1)
        # take the closest face
        closest_face = faces.data[np.argmin(distance)]

        face = preprocessing.inverse_transform(face)
        plt.imshow(
            face.reshape(32, 32).astype(np.uint8),
            cmap="gray",
        ...
        plt.imshow(
            closest_face.reshape(32, 32).astype(np.uint8),
            cmap="gray",
            interpolation="nearest",
            vmin=0,
            vmax=255,
