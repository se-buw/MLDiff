...
preprocessing = PCA(n_components=10)
...
if s.check() == sat:
    ...
    for m in all_smt(s, [clSvm, clDt]):
        digit = []
        for f in features:
            digit.append(float(s.model().eval(f, model_completion=True).as_fraction()))
        digit = preprocessing.inverse_transform(digit)
        print(f"{s.model()[clDt]} vs {s.model()[clSvm]}")
        plt.imshow(
            np.array(digit).reshape(8, 8),
